...
BEGIN
	DECLARE exit handler FOR sqlexception
		CALL p_log_test('RN-02', 'RN-02: No se permiten personas menores de 18', 'PASS');
...
BEGIN
	DECLARE exit handler FOR sqlexception
		CALL p_log_test('RN-04a', 'RN-04a: El ranking de un jugador debe de ser menor de 1000', 'PASS');
	
	CALL p_populate_db();
	INSERT INTO players (ranking) VALUES (2000);
	CALL p_log_test('RN-04a', 'ERROR: Se insertó un jugador con ranking mayor que 1000', 'FAIL');
END //
DELIMITER ;
...
BEGIN
	DECLARE exit handler FOR sqlexception
		CALL p_log_test('RN-04b', 'RN-04b: El ranking de un jugador no puede ser nulo', 'PASS');
